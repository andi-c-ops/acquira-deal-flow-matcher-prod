...
async function main() {
  ...
  const outputPath = required(process.env.GOOGLE_GMAIL_TOKEN_OUTPUT_FILE, "GOOGLE_GMAIL_TOKEN_OUTPUT_FILE");
  const port = Number(process.env.GOOGLE_GMAIL_AUTH_PORT ?? "8788");
  ...
  const server = createServer(async (request, response) => {
    ...
    if (error || !code) {
      response.writeHead(400, { "Content-Type": "text/plain" }).end("Gmail authorization was not completed. You can close this tab.");
      server.close();
      process.exitCode = 1;
      return;
    }
    ...
    try {
      ...
      response.writeHead(200, { "Content-Type": "text/plain" }).end("Gmail authorization is complete. You can close this tab and return to Codex.");
      console.log(`Gmail token saved to ${outputPath}`);
  ...
  server.listen(port, "127.0.0.1", () => {
    console.log("Open this URL in the Acquira Google account to authorize Gmail report delivery:");
